...
export function WebOffice2015() {
  ...
  this.WebSave = function () {
    ...
    this.WebSetMsgByName("fileSource", this.fileSource);
  ...
  this.WebSavePDF = function () {
    ...
    if (
      this.SAVEFILE(httpclient, this.getFilePath() + this.RecordID + ".pdf")
    ) {
  ...
  /*下载服务器上的文件并保存在本地 */
  this.WebGetPDFFile = function () {
    var LocalFile = "";
    var RemoteFile = this.RecordID + ".pdf";
    var exts = "PDF(*.pdf)|*.pdf";
    exts += "||";
    LocalFile = this.obj.FileSystem.FileSaveAs(exts, RemoteFile); //this.FileName 显示名称
    if (LocalFile != "") {
      var httpclient = this.obj.Http; //设置http对象
      httpclient.Clear();
      this.WebSetMsgByName("RECORDID", this.RecordID); //加载RecordID
      this.WebSetMsgByName("LOCALFILE", LocalFile); //取得本地文件名称
      this.WebSetMsgByName("REMOTEFILE", RemoteFile); //取得远程文件名称
      this.WebSetMsgByName("OPTION", "GETPDFFILE"); //发送请求LOADFILE
      httpclient.AddForm("FormData", this.GetMessageString()); //这里是自定义json 传输格式。
      this.WebClearMessage(); //清除所有WebSetMsgByName参数
      httpclient.ShowProgressUI = true; //显示进度条
      if (httpclient.Open(this.HttpMethod.Post, this.WebUrl, false)) {
  ...
  this.WebAreaProtect = function (BMarksName) {
    if (this.FileType == ".doc" || this.FileType == ".docx") {
      ...
      if (this.obj.ActiveDocument.ProtectionType == "-1") {
        if (!this.obj.ActiveDocument.BookMarks.Exists(BMarksName)) {
          this.Status =
            "文件内没有名称为'" + BMarksName + "'的书签 ,请先添加书签!";
          return false;
        }
...
//判断当前浏览器是否支持iWebOffice2015插件
export function isSupportOfficePlug() {
  var abc = getBroswer();
  var broswer = abc.broswer || "";
  var version = abc.version || 0;
...
export function KGBrowser() {
  ...
  this.getUUID = function () {
    if (this.uuid == "") this.uuid = this.kg_uuid(16, 32);
    return this.uuid;
  };
  ...
  this.openWindow = function (url, param, onlyONE) {
    ...
    $.ajax({
      type: "get",
      async: false,
      url: "/QueryOpen", //此代码ip固定，端口号与Edit页面该方法一致，其他固定。
      jsonp: "hookback",
      dataType: "jsonp",
      success: function (data) {
      ...
      error: function () {
        ...
        //    window.location.href = "/download/KGBrowserSetup.exe"; //安装KGBrower
        //}
      },
    });
  ...
  this.openWindowBrowser = function (url, param, onlyONE) {
    /*
     * uri 打开KGBrowser的uri
     * failCb 打开KGBrowser失败回调
     * successCb 打开KGBrowser成功回调
     */
    ...
    var link = "KGBrowser://$link:" + basePathRoot() + url;
    if (param != null) {
      link = "KGBrowser://$link:" + basePathRoot() + url + param;
    }
    setTimeout(function () {
      openUriWithInputTimeoutHack(
        link,
        function () {
          console.log("检测到：未安装KGBrowser");
          var answer = confirm(
            "未安装支持多浏览器应用程序是否点击安装\n提示：安装的时候360全部点击允许程序所有操作"
          );
          if (answer) {
    ...
    script.onerror = function (evt) {
      ...
      //debugger
      //if (kgbrowser.LongConnect && kgbrowser.LongConnect.name!='undefined') {
      //    location.href = link;
      //    kgbrowser.LongConnect();
      //}
      //else {
  ...
  this.LongConnect = function () {
    var strUrl = "/LongListen?id=" + kgbrowser.uuid;
    $.ajax({
      type: "get",
      async: false,
