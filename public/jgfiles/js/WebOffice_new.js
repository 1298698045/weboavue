...
function WebOffice2015() {
  ...
  this.dVersion = "5.0.0.14";
  this.dUpdateDate = "2020-01-19";
  //高级功能：全文检索、文档对比
  ...
  // ******************************************************************************************************************************//
  // ---------------------------------------------------对外接口-------------------------------------------------------------------//
  this.ajaxRequest = function (option) {
    // 1. 首先简单验证传进来的参数是否合法
      ...
      if (xhr.readyState === 4) {
        if (xhr.status === 200) {
          if (option.success && typeof option.success === "function") {
            option.success(xhr.getResponseHeader("JSESSIONID"));
          }
        } else {
    ...
    try {
      xhr.open(option.method, option.url, false); // true 代表是异步请求
  ...
  this.SessionEnable = function () {
    console.log("sessionenable 0");
    var sessionid = "";
    this.ajaxRequest({
      url: this.ServerUrl + "/OfficeServer?OPTION=JSESSIONID",
      method: "GET",
      type: "json",
      data: {},
    ...
    console.log("sessionenable 1 = " + sessionid);
    var MyCookie = "JSESSIONID=" + sessionid;
    this.obj.INetSetCookieEx(this.ServerUrl + "/OfficeServer", MyCookie);
  };

  this.SessionEnableJQuery = function () {
    console.log("SessionEnableJQuery");
    var sessionid = "";
    $.ajaxSettings.async = false;
    var strUrl = this.ServerUrl + "/OfficeServer";
    console.log(strUrl);
    $.get(strUrl, { type: "GETSESSIONID", param: "11111" }, function (data) {
  ...
  this.WebOpen2 = function (downloadLink) {
    ...
    if (
      httpclient.Open(this.HttpMethod.Get, this.ServerUrl + downloadLink, false)
    ) {
      if (httpclient.Send()) {
        if (this.hiddenSaveLocal(httpclient, this, false, false)) {
          var mSaveResult = this.WebOpenLocalFile(
  ...
  this.WebSave2 = function (callback) {
    ...
    var strUrl =
      jsWebOffice.WebUrl +
      "/AJAXServer?OPTION=MultiPartUpload&FILENAME=" +
      filename;
  ...
  this.WebSave = function () {
    ...
    if (this.WebSetAllowEmpty()) {
      this.GetSingleFileName(this.FileName);
      var mSavePath =
        this.getFilePath() + Math.random() * 100000 + this.SingleFileName;
    ...
    if (this.WebSetMaxFileSize(mSavePath)) {
      ...
      if (this.obj.Version == "12.9.0.1008") {
        //alert("12.9.0.1008");
        if (this.SAVEFILE_EXT(httpclient, mSavePath)) {
          var ISO = httpclient.GetResponseHeader("ISO"); // 获取返回值
          this.RecordID = httpclient.GetResponseHeaderExt(); // GetResponseHeaderExt会从服务器获取imagefileid
        ...
        if (this.SAVEFILE(httpclient, mSavePath)) {
          var ISO = httpclient.GetResponseHeader("ISO"); // 获取返回值
          this.RecordID = httpclient.GetResponseHeader("imagefileid"); // 服务器返回imagefileid,是泛微自身业务规则
          this.Status = "成功将文档保存到服务器";
          return true;
        } else {
          return false;
  ...
  this.WebSavePDF = function () {
    ...
    var mSavePath =
      this.getFilePath() + Math.random() * 100000 + this.RecordID + ".pdf";
    this.SaveAsPdf(mSavePath);
    if (this.SAVEFILE(httpclient, mSavePath)) {
  ...
  this.WebSaveAsHtml = function () {
    if (this.SaveAsHtml(this.getFilePath() + this.RecordID + ".html")) {
      ...
      if (
        this.SAVEFILE(httpclient, this.getFilePath() + this.RecordID + ".html")
      ) {
  ...
  this.WebUseTemplate = function () {
    ...
    var mSaveResult = this.WebSaveLocalFile(
      this.getFilePath() + this.iWebOfficeTempName
    ); //将当前文档保存下来
    if (!(mSaveResult == 0)) {
      this.Status = "保存本地文档失败！错误代码为：" + mSaveResult;
      return false;
  ...
  this.WebSaveImage = function (ImageType, ImageName) {
    if (
      ...
      var strDirPath = this.getFilePath() + this.RecordID + ".files"; //获取html files路径
      this.obj.ActiveDocument.SaveAs(
        iFilePath,
        10,
        false,
        "",
      ...
      var DirPath = new VBArray(
        this.obj.FileSystem.FindToDirAsFile(strDirPath)
      ).toArray(); //获取html文件夹里的内容
      var httpclient = this.obj.Http; // 设置http对象
      httpclient.Clear();
      this.WebSetMsgByName("RECORDID", this.RecordID);
      ...
      if (
        this.SAVEFILE(httpclient, this.getFilePath() + this.RecordID + ".htm")
      ) {
    ...
    if (
      ...
      if (httpclient.Send()) {
        if (httpclient.Status == 200) {
          if (
            this.hiddenSaveLocal(httpclient, this, false, false, this.Template)
          ) {
  ...
  this.WebInsertImage = function (
    BookMark,
    ImageName,
    Transparent,
    ZOrder //插入远程图片：BookMark插入的书签的位置;ImageName远程图片的名称
  ) {
  ...
  this.WebSaveBookMarks = function () {
    ...
    for (var i = 1; i <= BKCount; i++) {
      ...
      var BookValue =
        this.obj.ActiveDocument.Bookmarks.Item(BookName).Range.text;
        ...
        try {
          for (var i in jsonObj) {
            ...
            if (this.obj.ActiveDocument.BookMarks.Exists(BookName)) {
              var mBookRange =
                this.obj.ActiveDocument.Bookmarks.Item(BookName).Range;
  ...
  this.WebSaveLocal = function () {
    ...
    var ext = fileNamePath.substring(
      fileNamePath.length - 3,
      fileNamePath.length
    );
    if (fileNamePath == "") {
      return true;
  ...
  this.WebOpenLocal = function () {
    this.setShowDialog(
      this.ShowDialog.DialogOpen,
      this.getOpenDocSuffix(this.FileType)
    );
  ...
  this.WebSaveLocalFile = function (FileName) {
    try {
      ...
      if (this.FileType != ".pdf") {
        var saveState = this.Save(
          FileName,
          this.getOfficeVersion(),
          this.FileType.substring(1).toUpperCase()
        );
      } else {
        var saveState = this.Save(
          FileName,
    ...
    if (this.FileType == ".doc" || this.FileType == ".docx") {
      if (this.obj.ActiveDocument.ProtectionType == "-1") {
        if (!this.obj.ActiveDocument.BookMarks.Exists(BMarksName)) {
          ...
          this.obj.ActiveDocument.Application.Selection.MoveLeft(
            (Unit = 1),
            (Count = BMVLength)
          );
  ...
  //设置书签字体
  this.SetBookMarksFont = function (
    bMarksName,
    fColor,
    fSize,
    fName,
  ...
  this.WebSetRevision = function (Show, Track, Print, Tool) {
    ...
    strCustomUI =
      ...
      " </customUI>";
    if (
      this.getDocType(this.FileType) == this.DocType.WORD &&
  ...
  this.WebSetProtect = function (Boolean, password) {
    ...
    Boolean
      ? this.VBAProtectDocument(docType, password)
  ...
  this.WebDocumentCompare = function (FileName1, FileName2) {
    ...
    var saveFilePath =
      filePath + Math.round(Math.random() * 100000000) + ".doc";
    if (FileName1 == null || FileName1 == "") {
      if (FileName2 == null || FileName2 === "") {
        ...
        if (this.WebDownloadFile_int(FileName2)) {
          ...
          this.Status =
            "文件【" + FileName2 + "】下载失败，该文档可能在服务器不存在!";
          return false;
        }
      }
    }
    // 与指定的后台两个文档进行对比
      ...
      if (FileName2 == null || FileName2 == "") {
        ...
        if (this.WebDownloadFile_int(FileName1)) {
          ...
          this.Status =
            "文件【" + FileName1 + "】下载失败，该文档可能在服务器不存在!";
          return false;
        }
      }
    }
  ...
  /* 设置控件皮肤 注:颜色格式为BGR格式*/
  this.WebSetSkin = function (
    titleBarColor /* 控件标题栏颜色 */,
    ...
    CustomToolbarStartColor /* 自定义工具栏开始颜色 */,
    TitleBarTextColor /* 控件标题栏文本颜色 */
  ) {
    this.Status = "";
    var style = this.obj.Style;
    try {
      style.TitleBarColor = titleBarColor;
      if (TitleBarTextColor == undefined || TitleBarTextColor == "") {
        // 设置默认标题文字颜色
  ...
  /* 保护文档 */
  this.VBAProtectDocument = function (docType, password, pValue) {
    if (pValue == null || pValue == "" || pValue == undefined) pValue = 2;
    if (docType == this.DocType.WORD) {
      // word 保护模式
      if (this.obj.ActiveDocument.ProtectionType == "-1") {
        this.obj.ActiveDocument.Protect(pValue, false, password); //第一个参数： 3可以复制  2不能复制
        this.Status = "文档已被锁定";
        return true;
      } else {
        this.Status = "文档锁定失败";
      ...
      var sCount =
        this.WebObject.ActiveDocument.Application.ActiveWorkbook.Sheets.Count;
      for (var i = 1; i <= sCount; i++) {
        if (
          !this.WebObject.ActiveDocument.Application.ActiveWorkbook.Sheets.Item(
            i
          ).ProtectContents
        ) {
          this.WebObject.ActiveDocument.Application.Sheets.Item(i).Protect(
            password
          );
        } else {
          this.Status = "文档已锁定";
  ...
  this.VBAUnProtectDocument = function (docType, password) {
    ...
    if (docType == this.DocType.WORD) {
      ...
      var sCount =
        WebOffice.WebObject.ActiveDocument.Application.ActiveWorkbook.Sheets
          .Count;
      for (var i = 1; i <= sCount; i++) {
        if (
          WebOffice.WebObject.ActiveDocument.Application.ActiveWorkbook.Sheets.Item(
            i
          ).ProtectContents
        ) {
          WebOffice.WebObject.ActiveDocument.Application.Sheets.Item(
            i
          ).Unprotect(password);
        } else {
          this.Status = "文档未锁定";
    ...
    try {
      ...
      if (docType == this.DocType.WORD) {
        ...
        if (this.obj.ActiveDocument.BookMarks.Exists(Position)) {
          setTimeout(
            this.obj.ActiveDocument.Application.Selection.GoTo(
              -1,
              0,
              0,
              Position
        ...
        this.obj.Activate(true);
        this.obj.ActiveDocument.Application.Selection.InsertFile(
          FileName,
          "",
          false,
          false,
    ...
    if (this.FileType == ".doc" || this.FileType == ".docx") {
      ...
      if (this.obj.ActiveDocument.ProtectionType == "-1") {
        ...
        for (var i = 0; i <= mMarksName.length; i++) {
          ...
          this.obj.ActiveDocument.Application.Selection.Editors.Add(-1);
        }
      ...
      for (var i = 0; i <= mMarksName.length; i++) {
        if (this.obj.ActiveDocument.BookMarks.Exists(mMarksName[i])) {
          ...
          try {
            ...
            this.Status =
              "执行取消区域保护时出现错误，错误原因：" + e.description;
            return false;
          }
        } else {
  ...
  this.DownFilePath = function () {
    ...
    var NewDownFilePath;
    if (this.HiddenDirectory) {
      //将文档保存在隐藏目录
      NewDownFilePath =
        fs.GetSpecialFolderLocation(0x20) +
        this.DOWN +
  ...
  this.ModifyReview = function () {
    ...
    for (
      i = 1;
      i <=
      this.obj.ActiveDocument.Application.CommandBars.Item("Track Changes")
        .Controls.Count;
      i++
    ) {
      if (
        this.obj.ActiveDocument.Application.CommandBars.Item(
          "Track Changes"
        ).Controls.Item(i).ID == 1715
      )
        this.obj.ActiveDocument.Application.CommandBars.Item(
          "Track Changes"
    ...
    if (OfficeVersion != "11.0") {
      if (
        ...
        try {
          ...
          this.Status = e.description;
          alert(
            "保存PDF异常,可能Office没有保存PDF功能，建议安装SaveAsPDFandXPS插件"
          );
          return false;
        }
      } else if (
        ...
        try {
          this.obj.ActiveDocument.Application.ActiveSheet.ExportAsFixedFormat(
            0,
            FilePath,
            0
          );
  ...
  this.getOpenDocSuffix = function (fileType) {
    ...
    var exts;
    exts = "";
    if (this.isWPS() || !this.getOfficeVersion()) {
      // 如果是office2003是不支持x格式的文档
    ...
    if (httpclient.Open(this.HttpMethod.Post, this.WebUrl, false)) {
      ...
      if (httpclient.Send()) {
        if (httpclient.GetResponseHeader("MsgError") == "404") {
          // 判断服务器是否存在文件
          this.Status =
            "文档【" + fn + "】下载失败，请确认该文档在服务器上是否存在";
          httpclient.Clear();
          return false;
        }
        httpclient.Clear();
        if (this.hiddenSaveLocal(httpclient, this, false, false, fn)) {
          // 下载成功时
          this.Status = "文档下载成功";
          return true;
        } else {
  ...
  this.SAVEFILE_EXT = function (
    httpclient,
    FileName // 与后台发生交互OPTION值为：SAVEFILE
  ) {
    //alert(this.GetMessageString());
    //console.log(this.GetMessageString());
    httpclient.AddFormExt("FormData", this.GetMessageString());
    httpclient.AddFileExt("FileData", FileName); // 需要上传的文件
    this.WebUrl = this.ServerUrl + this.SaveServlet;
    if (httpclient.UploadFileUsingExt(this.WebUrl)) {
      //this.RecordID = httpclient.GetResponseHeaderExt();
  ...
  this.Save = function (FileName, is2003, FileType) {
    ...
    var SaveFalg = this.obj.Save(
      FileName,
      eval("this.DocTypeValue." + FileType),
      true
    );
    if (SaveFalg == 80) {
      return "文件路径无效";
  ...
  this.setEditType = function (type) {
    try {
      switch (type) {
        case 0:
        case "0":
          this.VBAProtectDocument(this.getDocType(this.FileType), "123", 2); //不可复制
          break;
        case 1:
        case "1":
          this.WebShow(false); //无痕
          break;
        case 2:
        case "2":
          this.WebShow(true); //有痕
          break;
        case 4:
        case "4":
          this.VBAProtectDocument(this.getDocType(this.FileType), "123", 3); //可复制
          break;
        default:
      }
    } catch (e) {
  ...
  this.EnableSignature = function (vaul) {
    try {
      this.obj.ActiveDocument.Application.COMAddIns.Item(
  ...
  this.AddWaterMark = function (WaterMarkNmae) {
    if (this.FileType == ".doc" || this.FileType == ".docx") {
      if (this.blnIE()) {
        var intPageTotal =
          this.WebObject.ActiveDocument.Application.ActiveDocument.BuiltInDocumentProperties(
            14
          );
      } else {
        var intPageTotal =
          this.WebObject.ActiveDocument.Application.ActiveDocument.BuiltInDocumentProperties.Item(
            14
          ).Value();
      }
      ...
      if (this.obj.ActiveDocument.ProtectionType == "-1") {
        ...
        if (WaterCount > 0) {
          for (var i = WaterCount; i >= 1; i--) {
            if (
              (selection.HeaderFooter.Shapes.Item(i).Name = WaterMarkNmae + i)
            ) {
      ...
      if (this.obj.ActiveDocument.ProtectionType == "-1") {
        ...
        for (var i = 1; i <= intPage; i++) {
          ...
          try {
            ...
            /////获取服务器上的图片///////
            selection.HeaderFooter.Shapes.AddPicture(
              ImagePath + ImageName,
              false,
              true
            ).Select(); //水印位置
  ...
  this.ParagraphSettings = function () {
    var paragraphFormat =
      this.WebObject.ActiveDocument.Application.Selection.ParagraphFormat;
    ...
    this.WebObject.ActiveDocument.Application.Selection.MoveLeft(
      (Unit = 1),
      (Count = 1)
    ); //返回定位最前端 ;
  };
  ...
  this.CleanBackground = function () {
    ...
    //清理文字
    var sfShading =
      this.WebObject.ActiveDocument.Application.Selection.Font.Shading;
    sfShading.Texture = 0;
  ...
  /* 本地书签插入图片 */
  this.BookMarkInsertImage = function (Path, BookMark, Transparent, ZOrder) {
    if (this.obj.ActiveDocument.BookMarks.Exists(BookMark)) {
      this.obj.ActiveDocument.Bookmarks.Item(BookMark).Select();
    }
    var SelectionImage =
      this.obj.ActiveDocument.Application.Selection.InlineShapes.AddPicture(
        Path
  ...
  this.OleDialogPrint = function (OLEFlag, bCancel) {
    if (OLEFlag == 1) {
      ...
      var exts;
      exts =
        "Word Files(*.doc;*.docx;*.docm;*.dot;*.dotx;*.dotm;*.rtf)|*.doc;*.docx;*.docm;*.dot;*.dotx;*.dotm;*.rtf";
      exts +=
        "|Excel Files(*.xls;*.xlsx;*.xlsm;*.xlt;*.xltx;*.xltm)|*.xls;*.xlsx;*.xlsm;*.xlt;*.xltx;*.xltm";
      exts +=
        "|PowerPoint Files(*.ppt;*.pptx;*.pptm;*.pot;*.potx;*.potm)|*.ppt;*.pptx;*.pptm;*.pot;*.potx;*.potm";
      exts += "|Project Files(*.mpp;*.mpt)|*.mpp;*.mpt";
      exts +=
        "|Visio Files(*.vsd;*.vdx;*.vss;*.vsx;*.vst;*.vtx;*.vsw;*.vdw;*.vsdx;*.vssx;*.vstx;*.vsdm;*.vssm;*.vstm)|*.vsd;*.vdx;*.vss;*.vsx;*.vst;*.vtx;*.vsw;*.vdw;*.vsdx;*.vssx;*.vstx;*.vsdm;*.vssm;*.vstm";
      exts += "|All File(*.*)|*.*";
      exts += "||";
  ...
  this.WebWordHighlight = function (word, coloridx) {
    this.obj.ActiveDocument.Application.Selection.WholeStory();
    this.obj.ActiveDocument.Application.Selection.Find.ClearFormatting();

    this.obj.ActiveDocument.Application.Selection.Find.Text = word;
    this.obj.ActiveDocument.Application.Selection.Find.Forward = true;
    this.obj.ActiveDocument.Application.Selection.Find.Execute();

    while (this.obj.ActiveDocument.Application.Selection.Find.Found == true) {
      this.obj.ActiveDocument.Application.Options.DefaultHighlightColorIndex =
        coloridx;
      this.obj.ActiveDocument.Application.Selection.Range.HighlightColorIndex =
        coloridx;
      this.obj.ActiveDocument.Application.Selection.Find.Execute();
    }
  };
...
function KGBrowser() {
  ...
  this.getUUIDA = function () {
    if (this.uuidA == "") {
      this.uuidA = this.kg_uuid(16, 32);
    }
    return this.uuidA;
  };

  this.getUUIDB = function () {
    if (this.uuidB == "") {
      this.uuidB = this.kg_uuid(16, 32);
    }
    return this.uuidB;
  };

  this.openWindowSPE = function (url, param, onlyONE) {
    // speed up 不做安装检测 , 好像效果不明显
  ...
  this.openWindow = function (url, isLongConnect, cookie) {
    var webUrl = basePath() + url;
    var params = getRequest(webUrl);

    webUrl += "$cookie=" + cookie;
    var that = this;
    // 检测KGBrowser是否安装
    $.ajax({
      type: "get",
      async: false,
      url: "http://127.0.0.1:9588/StartApp?weburl=" + webUrl, //此代码ip固定，端口号与Edit页面该方法一致，其他固定。
      jsonp: "hookback",
      dataType: "jsonp",
      success: function (data) {
        //建立常连接
        if (isLongConnect) {
          that.LongConnect(params.id);
        }
      },
      error: function () {
        alert("打开弹窗APP未知错误");
  ...
  this.LongConnect = function (id) {
    var that = this;
    var strUrl = "http://127.0.0.1:9588/LongListen?id=" + id;
    $.ajax({
      type: "get",
      async: false,
      ...
      dataType: "jsonp",
      success: function (data) {
        var jsonRet = eval(data);
        if (jsonRet.ret != "none") {
          var jsonMsg = eval(jsonRet.ret);
          console.log(jsonMsg);
          if (jsonMsg.action == "callback") {
            alert(decodeURIComponent(jsonMsg.data.msg));
          }
        }
        that.LongConnect(id);
      },
