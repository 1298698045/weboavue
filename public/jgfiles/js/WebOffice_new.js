...
//	   	}; 
	...
	this.WebSavePDF = function() 
	{
		...
		var mSavePath = this.getFilePath() + Math.random() * 100000 + this.RecordID + ".pdf";
...
/*	 文档保存到本地（有对话框）
	...
	this.VBAProtectDocument = function(docType, password,pValue) 
	{
		...
		{
			var sCount = this.WebObject.ActiveDocument.Application.ActiveWorkbook.Sheets.Count;
			for(var i=1;i<=sCount;i++){
				if(!this.WebObject.ActiveDocument.Application.ActiveWorkbook.Sheets.Item(i).ProtectContents){
					this.WebObject.ActiveDocument.Application.Sheets.Item(i).Protect(password);
				} else {
					this.Status = "文档已锁定";
					return false;
				}
	...
	this.VBAUnProtectDocument = function(docType, password) 
	{
		...
		if (docType == this.DocType.WORD) // word 保护模式
		{
			...
			var sCount = WebOffice.WebObject.ActiveDocument.Application.ActiveWorkbook.Sheets.Count;
			for(var i=1;i<=sCount;i++){
				if(WebOffice.WebObject.ActiveDocument.Application.ActiveWorkbook.Sheets.Item(i).ProtectContents){
					WebOffice.WebObject.ActiveDocument.Application.Sheets.Item(i).Unprotect(password);
				} else {
					this.Status = "文档未锁定";
					return false;
				}
    ...
    */
	...
	this.SAVEFILE_EXT = function(httpclient, FileName) // 与后台发生交互OPTION值为：SAVEFILE
	{
	...
	this.BookMarkInsertImage = function(Path,BookMark,Transparent,ZOrder) {
		...
		var SelectionImage = this.obj.ActiveDocument.Application.Selection.InlineShapes.AddPicture(Path);
		SelectionImage.Select();
		SelectionImage.PictureFormat.TransparentBackground = Transparent; //是否透明处理
		var ShapeImage = SelectionImage.ConvertToShape(); 	// 转为浮动型
		ShapeImage.WrapFormat.Type = 3; 
	...
	this.WebWordHighlight = function(word,coloridx){
		...
		{
			this.obj.ActiveDocument.Application.Options.DefaultHighlightColorIndex = coloridx;
			this.obj.ActiveDocument.Application.Selection.Range.HighlightColorIndex = coloridx;
			this.obj.ActiveDocument.Application.Selection.Find.Execute();
		}
	}
