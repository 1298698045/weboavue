                ...
                <div class="modalCenter">
                    <a-form ref="formRef" :model="formState" :label-col="labelCol">
                        ...
                        <a-form-item label="目录" name="column" :rules="[{ required: true, message: '请选择目录!' }]">
                            <a-tree-select v-if="props.objectTypeCode != '20021'" v-model:value="formState.column"
                                show-search style="width: 100%"
                                :dropdown-style="{ maxHeight: '400px', overflow: 'auto' }" placeholder="请选择目录"
                                allow-clear tree-default-expand-all :tree-data="treeData" tree-node-filter-prop="name"
                                :disabled="formState.column == '00000000-0000-0000-0000-000000002000' || props.objectTypeCode == '100311' || props.objectTypeCode == '100201'">
                                <template #title="{ name }">
                                    <span>{{ name }}</span>
                                </template>
                            </a-tree-select>
                            <a-tree v-if="props.objectTypeCode == '20021'" :expanded-keys="expandedKeys"
                                :auto-expand-parent="autoExpandParent" :tree-data="treeData" block-node
                                :fieldNames="fieldNames" :selectedKeys="selectedKeys" @select="onSelect"
                                @expand="onExpand">
                        ...
                        <a-form-item name="TemplateId" label="模板" :rules="[{ required: true, message: '请选择模板' }]">
                            <a-select v-model:value="formState.TemplateId" placeholder="请选择模板" filterable>
                                <a-select-option v-for="(item, index) in TemplateList" :value="item.value"
                                    :key="index">{{
                                        item.label
                                    }}</a-select-option>
                            </a-select>
...
const { businessUnitId, TemplateList, fieldName, selectedKeys, listData, menus, currentMenu, ModalTitle, sObjectName, userId, userName, top } = toRefs(data);
