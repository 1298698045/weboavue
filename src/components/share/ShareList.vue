<template>
    <div class="MeetingShareWrap" ref="TopicsLst">
          ...
          <a-table :columns="columns" :dataSource="listData" :scroll="{ y:tableHeight }" :pagination="false" @change="handleTableChange">
            <template #bodyCell="{ column, index,record }">
              ...
              <template v-if="column.key === 'AccessObjectTypeCode'">
                <div v-if="record.AccessObjectTypeCode=='8'" class="share-type">
                  <span class="share-type-img">
                    <img :src="require('@/assets/img/user/user.png')" alt="">
                  </span>
                  <span>用户</span>
                </div>
                <div v-if="record.AccessObjectTypeCode=='1036'" class="share-type">
                  <span class="share-type-img">
                    <img :src="require('@/assets/img/user/role.png')" alt="">
                  </span>
                  <span>角色</span>
                </div>
                <div v-if="record.AccessObjectTypeCode=='9'" class="share-type">
                  <span class="share-type-img">
                    <img :src="require('@/assets/img/user/group.png')" alt="">
                  </span>
                  <span>小组</span>
                </div>
                <div v-if="record.AccessObjectTypeCode=='10'" class="share-type">
                  <span class="share-type-img">
              ...
              <template v-if="column.key === 'Depth'">
                <div v-if="record.Depth=='2'">允许查看</div>
                <div v-if="record.Depth=='4'">允许查看和新建任务</div>
              </template>
      ...
      <AddShare :isShow="isShare" v-if="isShare"  @cancel="onSearch" :id="props.id" :title="props.title" :RegardingObjectTypeCode="props.RegardingObjectTypeCode" :RegardingObjectIdName="props.RegardingObjectIdName" />
    </div>
  </template>
  <script setup>
  ...
  const getQuery = () => {
    ...
    let filterQuery = '\nObjectId\teq\t' + (props.id || '')+'\nObjectTypeCode\teq\t'+(props.RegardingObjectTypeCode||'');
          proxy.$post(Interface.list2, {
              ...
              sort:'CreatedOn',
              order:'desc',
              displayColumns:'AccessObjectTypeCode,Name,Depth'
          }).then(res => {
              var list = [];
