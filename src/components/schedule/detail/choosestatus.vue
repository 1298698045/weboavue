        ...
        <a-dropdown>
            ...
            <template #overlay>
                <a-menu ref="elDropdown" class="lang-select-dropdown">
                    <a-menu-item :command="item.statusId+'&'+item.name+'&'+item.statusCategoryId" v-for="item in statusdata" :key="item.id" :class="{'selected':item.statusId==statusId}" @click="getdata(item.id)">
...
.selected,.el-dropdown-menu__item:hover {
