    ...
    <div class="MeetingShareWrap" ref="TopicsLst">
        <div class="panel" :style="{ height: height + 'px' }">
            ...
            <div class="panel-bd panel-bd1">
                <a-table :columns="props.columns" :dataSource="listData" :scroll="{ y: tableHeight }"
        ...
        <common-form-modal :isShow="isCommon" v-if="isCommon" @cancel="handleCommonCancel"
            :title="recordId ? '编辑' : '新建'" @load="onSearch" :id="recordId" :objectTypeCode="objectTypeCode"
        ...
        <AddShare :isShow="isShare" v-if="isShare" @cancel="onSearch" :id="props.id" :title="props.title"
...
import AddShare from "@/components/share/AddShare.vue";
...
const { displayColumns, listData, height, searchVal, OwningBusinessUnitName, pagination, tableHeight, recordId, objectTypeCode, sObjectName, isDelete, isCommon, isTaskDetail, isShare, deleteDesc, external, isRadioUser, CheckinStatus, StatusCode, Checkin, Checkin1, Checkin2, isRadioDept } = toRefs(data);
const getQuery = () => {
    ...
    let filterQuery = '';
    if (props.RegardingObjectIdName) {
        filterQuery = '\n' + props.RegardingObjectIdName + '\teq\t' + (props.id || '');
    }
...
onMounted(() => {
    ...
    if (props.columns && props.columns.length) {
        ...
        for (var i = 0; i < props.columns.length; i++) {
            if (props.columns[i].key != 'index' && props.columns[i].key != 'Action') {
                list.push(props.columns[i].key)
            }
        }
        data.displayColumns = list.join(',');
        getQuery();
    }
