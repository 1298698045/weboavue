<template>
          ...
          <div class="modalCenter" :style="{ height: height + 'px!important' }">
            <div class="searchBox">
                <div class="searchForm" :class="{'active':isAdvance}">
                    ...
                    <a-button class="searchBtn" @click="advancedSearch">
                        高级搜索
                    </a-button>
                </div>
  ...
  const {
    title,
    height, columns, isCollapsed, tableHeight, searchVal, isAdvance
  } = toRefs(data);
...
const onSearch = (e) => {
  ...
  const getConfig = () => {
    proxy.$get(Interface.lookupfilter,{
        ...
        gridRef.value.loadGrid({objecttypecode: props.objectTypeCode});
    })
  }
  getConfig();
  const handleSubmit = () => {
