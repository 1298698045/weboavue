            ...
            <div class="section">
              <div class="sectionRow">
                <div class="sectionItem">
                  <a-form-item name="StateCode" label="合同状态" :rules="[{ required: true, message: '请选择' }]">
                    <!-- <a-input v-model:value="formState.StateCode" class="inputClass"></a-input> -->
                    <a-select
...
const { title, height, StateCodeList } = toRefs(data);
const getData = () => {
  let obj = {
    actions: [
      {
...
const getPickerList = () => {
  ...
  proxy.$post(Interface.pickListValues, obj).then((res) => {
    if(res&&res.actions&&res.actions[0]&&res.actions[0].returnValue&&res.actions[0].returnValue.picklistFieldValues){
      let picklistFieldValues = res.actions[0].returnValue.picklistFieldValues;
      data.StateCodeList = picklistFieldValues.StateCode?picklistFieldValues.StateCode.values:[];
    }
  });
};
const handleSubmit = () => {
