<template>
  <div class="LeaderFullCalendarWrap">
    <div ref="LeaderFullCalendarRef">
      <FullCalendar
        class="fullCalendar"
        ref="fullCalendarRef"
                  ...
                  <div class="meetingBody">
                    ...
                    <div class="meetingInfo">
                      ...
                      <div class="meetingInfoItem">
                        分配人：
                        <span class="TelePhone">{{
                          arg.event.extendedProps.CreatedByName || ""
                        }}</span>
                      </div>
                    </div>
                    <div class="meetingInfo">
                    ...
                    <div class="meetingInfo">
                      <a-button
                        type="link"
                        @click.stop="handleDetail(arg.event)"
                        >更多详细信息</a-button
                      >
                    </div>
                  </div>
...
onMounted(() => {
  data.height = LeaderFullCalendarRef.value.scrollHeight;
  ...
  if (start && end && start != end) {
    ...
    if (text == "移动事件或者拓展事件" && info[0]) {
      let e = {
        ...
        extendedProps: {
          CalendarActionsConfig: {
            isCalendarEditable: data.CalendarActionsConfig.isCalendarEditable,
          },
        },
      };
      if (!info[0].id) {
  ...
  proxy.$post(Interface.leaderSchedule.list, obj).then((res) => {
    ...
    if (
      ...
      peopleList.forEach((item, index) => {
        ...
        data.CalendarActionsConfig =
          peopleList[0].calendar.CalendarActionsConfig;
      });
      ...
      for (var i = 0; i < peopleList.length; i++) {
        ...
        scheduleItems.forEach((item, index) => {
          ...
          let event = {
            ...
            backgroundColor: ite.calendar.Color
              ? ite.calendar.Color
              : colors[0], // 该事件的背景颜色
            borderColor: ite.calendar.Color ? ite.calendar.Color : colors[0], // 该事件的边框颜色
            textColor: getComplementaryColor(
              ite.calendar.Color ? ite.calendar.Color : colors[0]
            ), // 该事件的文字颜色
          ...
          if (
            fullCalendarRef.value.getApi().view.calendar.getEventById(item.Id)
          ) {
            fullCalendarRef.value
              .getApi()
              .view.calendar.getEventById(item.Id)
              .remove();
          }
