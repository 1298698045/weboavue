        ...
        <a-table :scroll="{ x: '1400px' | true }"  :columns="columns" :data-source="dataList" :pagination="false">
            <template #bodyCell="{ column, record, index }">
                <template v-if="column.dType === 'S'">
                    {{ record.fields[column.key].value }}
                </template>
                <template v-else-if="column.dType === 'N'">
                    {{ record.fields[column.key].value }}
                </template>
                <template v-else-if="column.dType === 'U' || column.dType === 'O'">
                    {{ record.fields[column.key].displayValue }}
                </template>
                <template v-else-if="column.dType === 'D'">
                    {{ record.fields[column.key].value }}
                </template>
                <template v-else-if="column.dType === 'L' || column.dType === 'LT' || column.dType === 'DT'">
                    <span v-if="column.key=='StatusCode'">
                        {{ record.fields[column.key].value == 0 ? '未接受' : '已接受'}}
                    </span>
                    <span v-else-if="column.key=='CheckinStatus'">
                        {{ record.fields[column.key].value == 0 ? '未签到' : '已签到'}}
                    </span>
                    <span v-else>
                        {{ record.fields[column.key].value }}
                    </span>
...
<script setup>
    ...
    const data = reactive({
        ...
        localId: "",
        recordVal: {},
        moduleName: "MeetingAudience"
    });
    const field = {};
    const { columns, dataList, selectPickers, search, isRadioDept, isRadioUser, localId, recordVal, moduleName } = toRefs(data);
    // S
    ...
    const getRelated = () => {
        proxy.$get(Interface.meeting.relatedrecords2,{}).then(res=>{
            console.log("res",res);
            ...
            let list = recordIds[data.moduleName].recordIds;
            list.forEach(item=>{
                data.dataList.push(records[item][data.moduleName].record);
            })
