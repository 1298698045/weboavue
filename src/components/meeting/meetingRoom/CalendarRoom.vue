<template>
    <div class="CalendarRoomWrap">
        <div class="rightBox">
            ...
            <div class="calendarBody" ref="contentRef" v-if="isfresh">
                <!-- <DayCalendar ref="DayCalendarRoomWrap" v-if="calendarType==0" :currentTime="currentTime" :startDateTime="startTime" :endDateTime="endTime" :calendarType="formState.type" @openNew="handleOpenNew" @handleDetail="handleDetail" @openEdit="handleOpenEdit" @handleDelete="handleDelete" />
                ...
                <MeetingRoomFullCalendar ref="FullCalendarRoomWrap" v-if="isfresh" :calendarView="calendarView"
        ...
        <CommonConfirm v-if='isConfirm' :isShow="isConfirm" :text="confirmText" :title="confirmTitle"
            @cancel="isConfirm = false" @ok="deleteOk" :id="meetingId" />
    </div>
</template>
<script setup>
import {
...
const MonthCalendarRoomWrap = ref(null);
const DayCalendarRoomWrap = ref(null);
...
const FullCalendarRoomWrap = ref(null);
const calendarTypeChange = (e) => {
    ...
    if (e == 2) {
        ...
        // nextTick(()=>{
        //     MonthCalendarRoomWrap.value.getQuery();
        // })
    }
    else if (e == 1) {
        ...
        // nextTick(()=>{
        //     DayCalendarRoomWrap.value.getQuery();
        // })
    }
    nextTick(() => {
        FullCalendarRoomWrap.value.getQuery();
    })
}
const backFn = (list) => {
    var len = list.length;
