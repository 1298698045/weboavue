    ...
    resourceAreaColumns: [
      {
        ...
        cellContent: function (info) {
          ...
          let ManageIdName =
            info.resource._resource.extendedProps.ManageIdName || "暂无";
          let MobilePhone =
            info.resource._resource.extendedProps.MobilePhone || "暂无";
          let innerText =
            `<div class="meetingRoomItem">
                            <div class="meetingRoomItemHover" style="top:` +
            top +
            `px;left:` +
            left +
            `px;">
            ...
            `
                            ...
                            <div class="meetingRoomItemTitle">` +
            Name +
            `</div>
                        </div>`;
...
const getQuery = () => {
  ...
  if (
    ...
    document.getElementsByClassName("fc-scrollgrid-sync-inner").length
  ) {
    for (
      var i = 0;
      i < document.getElementsByClassName("fc-scrollgrid-sync-inner").length;
      i++
    ) {
    ...
    if (
      ...
      roomList.forEach((item, index) => {
        ...
        item.meetingItems.forEach((item1, index1) => {
          ...
          item1.StartDateTime = item1.StartDateTime
            ? dayjs(item1.StartDateTime).format("YYYY-MM-DD HH:mm")
            : "";
          item1.EndDateTime = item1.EndDateTime
            ? dayjs(item1.EndDateTime).format("YYYY-MM-DD HH:mm")
            : "";
