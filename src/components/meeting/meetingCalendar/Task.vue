<template>
    <div class="taskWrap" ref="TopicsLst">
      <div class="panel" :style="{ height: height + 'px' }">
        <div class="panel-head">
          <div class="panel-title">会议任务</div>
          <div class="panel-btn">
      ...
      <AddTask :isShow="isAddTask" :id="recordId" v-if="isAddTask" :paramsTime="''" @cancel="isAddTask=false" :objectTypeCode="'4200'" :entityApiName="'ActivityPointer'" @selectVal="onSearch" :calendarType="''" :RegardingObjectTypeCode="props.RegardingObjectTypeCode" :RegardingObjectId="props.id" :RegardingObjectIdName="props.RegardingObjectIdName" :BgColor="''" />
      <TaskDetail ref="TaskDetailModal" :isShow="isTaskDetail" v-if="isTaskDetail" :id="recordId" @cancel="isTaskDetail=false" @selectVal="onSearch" @handleDelete="handleDelete" @edit="handleEdit" :RegardingObjectIdName="props.RegardingObjectIdName" />
    </div>
  </template>
  <script setup>
  ...
  import TaskDetail from "@/components/meeting/TaskDetail.vue";
  ...
  const { listData, height,searchVal,OwningBusinessUnitName,pagination,tableHeight,recordId,objectTypeCode,sObjectName,isDelete,isCommon,isTaskDetail,isAddTask,deleteDesc,external,isRadioUser,CheckinStatus,StatusCode,Checkin,Checkin1,Checkin2,isRadioDept } = toRefs(data);
  const getQuery = () => {
    ...
    if(data.Checkin2){
      filterQuery+='\nCheckin\tle\t'+data.Checkin2;
          ...
          proxy.$post(Interface.list2, {
              ...
              sort:'ScheduledStart',
              order:'desc',
              displayColumns:'IssueType,Subject,StateCode,ScheduledStart,OwningUser,ScheduledEnd,CreatedBy,CreatedOn'
          }).then(res => {
              var list = [];
  ...
  const onSearch = (e) => {
    data.isDelete=false;
    data.isAddTask=false;
    data.isTaskDetail=false;
    getQuery();
  };
  const onClear = (e) => {
    data.searchVal='';
  ...
  .peopleHeader .left .ant-picker{
    ...
    .taskWrap .panel-top .ant-card{
      border-radius: 4px !important;
    }
    .taskWrap .ant-card:first-child{
      margin-left: 0 !important;
    }
    .taskWrap .ant-card:last-child{
