...
<a-button class="ml5">移动</a-button>
                        ...
                        <div class="timerRow">
                            <a-tooltip placement="top" :title="detail.starEmail * 1 == 1 ? '点击取消重要邮件' : '点击标记重要邮件'">
                                <span @click.stop="handleStar(detail.starEmail * 1 == 1 ? 0 : 1)">
                                    <i v-if="detail.starEmail * 1 == 1" class="iconfont icon-shoucangyoujian"
                            ...
                            <div class="inboxFileItem" v-for="(item, index) in attachments" :key="index">
                                <div class="leftImg">
                                    ...
                                    <img src="/src/assets/img/filetype/PPT.png"
                                        v-else-if="item.fileExtension == 'ppt' || item.fileExtension == '.ppt'" />
                                    ...
                                    <div class="fileOptionShow" :title="(item.name || '')">
                                        <div class="btns">
                                            <a-tooltip title="保存到文档" placement="top">
                                                <a-button class="btn square default" title="保存到文档"
                                            ...
                                            <a-tooltip title="下载" placement="top">
                                                <a-button class="btn square default" @click="downloadFile(item)"
                                                    title="下载">
        ...
        <ChangeLabel :isShow="isLabel" v-if="isLabel" :id="props.emailId" @cancel="isLabel = false" />
        <NewContent v-if="isNewContent" :isShow="isNewContent" :name="fileName" :treeData="gData"
    ...
    proxy.$get(Interface.email.getAttachments, {
        ...
        if (res && res.actions && res.actions[0] && res.actions[0].returnValue && res.actions[0].returnValue) {
            ...
            for (var i = 0; i < data.attachments.length; i++) {
                ...
                data.attachments[i]['downloadUrl'] = item.downloadUrl && item.downloadUrl != 'undefined' ? item.downloadUrl : (item.fileLocation || '');
                data.attachments[i]['viewUrl'] = item.viewUrl && item.viewUrl != 'undefined' ? item.viewUrl : (item.fileLocation || '');
                let name = item.name || '';
                if (name) {
...
const openControlViewFile = (id, username, type, link, name) => {
  ...
  window.open('/#' + link + "&entityName=MailAttachBase", '', 'height=' + mhtmlHeight + ',width=' + mhtmlWidth + ',top=' + iTop + ',left=' + iLeft + ',toolbar=no,menubar=yes,scrollbars=no,resizable=yes, location=no,status=no');
};
//下载附件
