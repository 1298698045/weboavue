  ...
  <div class="AddGroup" ref="AddGroup">
    ...
    >
      <template #title>
        ...
        <div v-if="step == 1">上载组照片</div>
        <div v-if="step == 2">管理成员</div>
      </template>
                ...
                <div class="sectionItem">
                  ...
                  >
                    <a-select
                      ...
                      @search="
                        (e) => {
                          searchlookup(e, 'SystemUser', 'OwningUser');
                        }
                      "
                    ...
                    >
                      <template #suffixIcon></template>
                      <a-select-option
                        v-for="(option, optionIdx) in search.OwningUser"
                        :key="optionIdx"
                        :value="option.ID"
              ...
              <div class="sectionRow">
                <div class="sectionItem">
                  <a-form-item name="ImportSequenceNumber" label="顺序">
                    <a-input
                      v-model:value="formState.ImportSequenceNumber"
                    ></a-input>
                  </a-form-item>
                </div>
              </div>
            ...
            <div class="section">
              <div class="sectionRow">
                <div class="sectionItem">
                  ...
                  >
                    <a-select
                      v-model:value="formState3.member.Id"
                      ...
                      @search="
                        (e) => {
                          searchlookup(e, 'SystemUser', 'member');
                        }
                      "
                      @dropdownVisibleChange="
                        (e) => {
                          searchlookup('', 'SystemUser', 'member');
                        }
                      "
                    >
                      <template #suffixIcon></template>
                      <a-select-option
                        v-for="(option, optionIdx) in search.member"
                        :key="optionIdx"
                        :value="option.ID"
                        >{{ option.Name }}</a-select-option
                      >
              ...
              <div class="sectionRow">
                <div class="sectionItem addMember">
                  <a-select v-model:value="data.RoleCode">
                    <a-select-option
                      v-for="(item, index) in RoleCodeList"
                      :value="item.value"
                      :key="index"
                      >{{ item.label }}</a-select-option
                    >
                  </a-select>
                  <a-button @click="AddPeople" type="primary"
                  ...
                  >
                    <template #bodyCell="{ column, index, record }">
                      ...
                      <div v-if="column.key == 'AvatarImg'">
                        <img
                          :src="text || require('@/assets/img/avatar-r.png')"
                          alt=""
      ...
      <template #footer>
        <div>
          <a-button
            @click="handleCancel"
            class="cancelmodel AddGroupCancel"
            v-if="step != 2"
            >取消</a-button
          >
          <a-button
            ...
            >保存并继续</a-button
          >
          <a-button type="primary" @click.prevent="handleNext" v-if="step == 1"
            >下一步</a-button
          >
          <a-button
            type="primary"
...
const getDetail = () => {
  ...
  proxy.$post(Interface.detail, obj).then((res) => {
    if (res && res.actions && res.actions[0]) {
      ...
      formState.ImportSequenceNumber = record.ImportSequenceNumber
        ? record.ImportSequenceNumber.value
        : "";
      ...
      searchlookup("", "SystemUser", "OwningUser");
      // let url =
      //   record.AvatarUrl && record.AvatarUrl.value
            ...
            params: {
              recordInput: {
                ...
                fields: {
                  ...
                  Notice: item.Notice,
                  ImportSequenceNumber:
                    item.ImportSequenceNumber || item.ImportSequenceNumber == 0
                      ? item.ImportSequenceNumber * 1
                      : "",
...
const getQuery = () => {
  ...
  proxy
    .$post(Interface.list2, {
      ...
      order: "ASC",
      displayColumns:
        "RegardingObjectIdName,AvatarImg,FullName,BusinessUnitId,RoleCode,MobilePhone,EMailAddress,WorkStatus",
    })
    .then((res) => {
      var list = [];
...
onMounted(() => {
  ...
  data.tableHeight = document.documentElement.clientHeight - 400;
