        ...
        <div class="sectionable-table">
                    ...
                    <a-dropdown :trigger="['click']">
                        ...
                        <template #overlay>
                            <a-menu class="fh-menu" style="width: 136px;">
                              <a-menu-item key="1">添加筛选逻辑</a-menu-item>
            ...
            <div class="sectionable-table-section-content">
                <div class="sectionable-table-group">
                    <div class="sectionable-table-group-title">
                        <a-select placeholder="添加筛选器..." v-model:value="field" show-search :filterOption="filterOptionGroup" showSearch  style="width: 100%;" @change="changeField">
                            <template #suffixIcon>
                    ...
                    <div class="sectionable-table-group-content">
                        <ul class="section-table-rows-container">
                            <!-- <li class="sectionable-table-row" v-for="item in 5"  @click="handleEditForm(item)">
                                            ...
                                            <div class="filterContainer">
                                                <div class="filters__item">
                                                    <button class="button_reset">
                                                        <span class="show text-body_small">向我显示</span>
                                                        <span class="show"><span class="filterPredicate"> 全部业务机会</span></span>
                                                    </button>
                                                </div>
                                            </div>
                                        </div>
            ...
            <section class="portal-positioned" ref="popverRef" :style="{top:top+'px'}" v-if="isEditForm">
                <header class="popover__header">
                    <h2 class="text-heading_small">筛选</h2>
                    ...
                    <div class="form_stacked">
                        <a-form :model="formState" ref="formRef">
                            <a-form-item name="field" label="字段" :labelWrap="true">
                                ...
                                <a-select placeholder="添加筛选器..." v-model:value="formState.field" :filter-option="false" showSearch style="width: 100%;">
                                    <template #suffixIcon>
                                        <svg class="fh-input__icon fh-input__icon_left" focusable="false" data-key="search" aria-hidden="true" viewBox="0 0 520 520" part="icon"><g><path d="M496 453L362 320a189 189 0 10-340-92 190 190 0 00298 135l133 133a14 14 0 0021 0l21-21a17 17 0 001-22zM210 338a129 129 0 11130-130 129 129 0 01-130 130z"></path></g></svg>
                                    </template>
                                    <a-select-opt-group v-for="(item, index) in categories" :label="item.label">
...
<script setup>
    ...
    watch(()=>data.filterList, (newVal, oldVal)=>{
        ...
        const isBook = newVal.every(item=>item.isComplete);
        if(isBook){
    ...
    watch(()=> props.describeReport.reportMetadata.reportFilters, (newVal, oldVal)=> {
        loadFn();
    ...
    const handleEditForm = (item, index) => {
        ...
        // console.log("itemRefs", itemRefs[index].el.offsetTop);
        // console.log("itemRefs[index].el", itemRefs[index].el);
