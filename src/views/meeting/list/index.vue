...
const { layoutName, entityType, sObjectName, hightSearchParams, isCollapsed, tableHeight, fieldNames, tabs, activeKey, isModal, isCirculation, searchVal,
...
watch(() => route, (newVal, oldVal) => {
  if (gridRef && gridRef.value && gridRef.value.loadGrid != 'undefined' && !route.params.sObjectName) {
    if (route.path == '/lightning/o/MyMeeting/list') {
      //getTreeData();
      data.queryParams = {
        filterId: '',
        objectTypeCode: '5000',
      ...
      data.entityType = '00V';
      data.layoutName = 'MyMeetingAdmin'
      setTimeout(function () {
        getTabs();
      }, 1000)
    }
