    ...
    <div class="attdWrap">
        <div class="attdHead">
            <div class="attdTop">
                ...
                <div class="col_right">
                    <a-button>查询请假</a-button>
                    <a-button class="ml10">设置班次</a-button>
                    <a-button class="ml10">可打印视图</a-button>
                    <a-button class="ml10">导出Excel</a-button>
                </div>
            </div>
            <div class="attdBottom">
                ...
                <div class="col_right">
                    ...
                    <a-button class="ml10" @click="handleClearAttdRecord">删除</a-button>
                    <div class="btnGroup ml10">
                        <a-button @click="handleEmpIn">跟组排班</a-button>
                        <a-button @click="handleEmpOut">批量排班</a-button>
                        <a-button @click="handleSort">模板排班</a-button>
                        <a-button @click="handleSort">换班</a-button>
                        <a-button @click="handleSort">复制排班</a-button>
                    </div>
                    <div class="btnGroup ml10">
                    ...
                    <a-button type="primary" class="ml10" @click="handleSubmitAttd(1)">提交</a-button>
                </div>
            </div>
        </div>
        <div class="attdBd" ref="bdRef">
            ...
            <div class="attd-classcheck" ref="attdCheckRef">
                <div class="col">
                    ...
                    <span class="deptTag" :class="{'active':deptCurrent.id == item.id}" v-for="(item, index) in objData.Units" :key="index" @click="handleDeptTab(item)">{{item.name}}</span>
                </div>
            </div>
            <div class="attd-departmentcheck" ref="attdDeptRef">
                <span class="typeLabel">班次：</span>
                <div>
                    <a-popconfirm trigger="hover" :showCancel="false" :showOk="false" v-for="(item,index) in AttendTypes" :key="index">
                        ...
                        <span class="typeItem" :class="{'active':currentType.Id==item.Id}" @click="handleAttdTypes(item, index)">{{item.Name}}</span>
                    </a-popconfirm>
                </div>
            </div>
            <div class="attdTable">
                ...
                <div class="attend-table-body">
                    <div class="attend-table-body-left" :style="{height:(height-10)+'px'}" ref="leftRef">
                        <div>
                            <a-checkbox-group v-model:value="employeeSelect">
                                <div class="attend-table-row" :style="{height:item.height+'px'}" v-for="(item, index) in listData" :key="index">
                                    <div class="attend-table-td" style="min-width: 25px; width: 25px; height: 100%; line-height: 30px;">
                                    ...
                                    <div class="attend-table-td" style="width: 75px; height: 100%; line-height: 30px;">        
                                        <div>{{item.Name}}</div>    
                                    </div>
                                    <div class="attend-table-td" style="width: 75px; height: 100%; line-height: 30px;">        
                    ...
                    <div class="attend-table-body-right" :style="{height:height+'px'}" ref="bodyRef">
                        <div>
                            <div class="attend-table-row" :style="{width:width+'px',height:item.height+'px'}" v-for="(item, index) in listData" :key="index"  :ref="(el) => {setRowRef(el, item, index);}">
                                <div class="attend-table-td" style="width: 75px;" :style="{height:item.height+'px'}" v-for="(row, idx) in item.AttendData" :key="idx" @click.stop="handleSelectCol(item, index, row, idx)">        
                                    <div>
                                        <p v-for="(typeItem, typeIdx) in row.Shifts" :key="typeIdx">
                                            {{typeItem.Name}}
                                        </p>
...
<script setup>
    ...
    const mouseDown = (e) => {
        ...
        if(e.x < 200 || e.y < offsetTop.value){
            startMove.value = false;
        }
        const {x, y} = e;
        // areaPoint.startX = x - 200;
    ...
    const { time, objData, AttendTypes, listData, 
