                ...
                <div class="tabContainer containerForm" v-if="activeKey==0">
                    ...
                    <div class="rightAside">
                        <div class="panel">
                            ...
                            <div class="panel-bd">
                                <a-collapse v-model:activeKey="current" :expand-icon-position="expandIconPosition">
                                    <a-collapse-panel :header="item.Name" v-for="(item,index) in fileCategorys" :key="index">
                                        <!-- <p>{{ text }}</p> -->
                                ...
                                <div class="files">
                                    <div class="fileItem">
                                        ...
                                        <div class="rInfo">
                                            <p class="name rowEllipsis">觉得卡拉利空打击了的空间阿斯利康大家大家按时大家按时按时</p>
                                            <div class="optionLink">
                                                <span class="link">查看</span> ·
                                                <span class="link">下载</span>
                                            </div>
                                            <div class="time">
                                                2023/11/7 10:10 · 36.7k
                                            </div>
                                        </div>
                                        <div class="icon">
...
<script setup>
    ...
    const { tabs, activeKey, id, objectTypeCode, detail, isNotes,
        isStatus, isRemind, isAddClass, isUserModal, fileCategorys, files } = toRefs(data);
    const changeTabs = (e) => {
        data.activeKey = e;
    }
    const current = ref([]);
    ...
    getDetail();
    const getFileClass = () => {
        proxy.$get(Interface.information.attachmentCategory,{
            ObjectTypeCode: data.objectTypeCode
        }).then(res=>{
    ...
    getFileClass();
    const getFiles = () => {
        proxy.$get(Interface.information.files,{
            id: data.id
        }).then(res=>{
...
<style lang="less" scoped>
    ...
    :deep :where(.css-dev-only-do-not-override-kqecok).ant-collapse>.ant-collapse-item:last-child, :where(.css-dev-only-do-not-override-kqecok).ant-collapse>.ant-collapse-item:last-child>.ant-collapse-header{
