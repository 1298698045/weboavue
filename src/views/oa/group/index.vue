                    ...
                    <a-table style="height: 100%;" :scroll="{ y: tableHeight }" :dataSource="dataSource"
                        ...
                        <template #bodyCell="{ column, text, record }">
                            <div v-if="column.key == 'AvatarUrl'">
                                <img :src="record.AvatarUrl" @error="handleImageError(record)" alt=""
...
const getQuery = () => {
    ...
    proxy.$post(url, d).then(res => {
        let list = [];
        if (res && res.nodes && data.selectedKeys[0] != 'join') {
        ...
        else if (res && res.actions && res.actions[0] && res.actions[0].returnValue && data.selectedKeys[0] == 'join') {
...
const handleImageError = (record) => {
    record.AvatarUrl = data.defaultImg;
    return record;
};
onMounted(() => {
