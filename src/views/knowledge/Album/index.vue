    ...
    <div class="headerBar">
      ...
      <div class="headerRight">
        <a-upload
          ...
          v-if="
            leftName != '回收站' &&
            data.BreadCrumbList.length &&
            Privileges.canAdd
          "
          multiple
          name="file"
          :customRequest="changeRequest1"
        ...
        <div class="rightHeader">
          ...
          <div class="rBtns">
            ...
            <a-button
              ...
              >查询</a-button
            >
            <a-button class="ml10" @click="onClear" v-if="!isChangeName"
              >重置</a-button
            >
        ...
        <div class="tableWrap" ref="tablelist">
          <div class="card-content" v-show="data.type == '1'">
            <div
              ...
              :key="index"
              @click.stop="handleOpenFolder(item.id, item.Name)"
              @mouseover="handleMouseOver(item.id, item.Name)"
            >
              <img :src="require('@/assets/img/filetype/Folder.png')" />
              <div class="add-addtext" :title="item.Name" v-if="!isChangeName">
                {{ item.Name }}
              </div>
              ...
              <div class="iconBox content-item-iconBox">
                <div class="popup">
                  ...
                  <div
                    ...
                    v-if="Privileges.canAdd"
                  >
                    编辑
                  </div>
                  <div
                    class="option-item"
                    @click.stop="handleDepth(item)"
                    v-if="Privileges.canAdmin"
                  >
                    设置权限
                  </div>
                  <div
                    class="option-item"
                    @click.stop="handleDelete(item.id, 'folder')"
                    v-if="Privileges.canDelete"
                  >
                    删除
                  </div>
                </div>
            ...
            <div
              ...
              :key="index"
              @click="handleOpenFile(item, index)"
            >
              <img
                :src="
                  item.ThumbnailUrl ||
                  require('@/assets/img/filetype/defaultImg.png')
                "
              />
              <div class="add-addtext" :title="item.Name" v-if="!isChangeName">
                {{ item.Name }}
              ...
              <div class="add-addtime">{{ item.CreatedOn }}</div>
              <a-checkbox class="fileCheckBox" v-model:checked="item.checked" @click.stop :style="{visibility:item.checked?'visible':'hidden'}" @change="changeCheckbox(item)" v-if="leftName == '回收站'"></a-checkbox>
              <div class="iconBox content-item-iconBox">
                <div class="popup">
                  <div
                    class="option-item"
                    @click.stop="handleOpenFile(item, index)"
                    :num="index"
                    v-if="Privileges.canRead && leftName != '回收站'"
                  >
                    预览
                  </div>
                  <div
                    ...
                    v-if="Privileges.canDelete && leftName != '回收站'"
                  >
                    删除
                  </div>
                  <div
              ...
              <div v-if="column.key == 'Action'">
                <div class="iconBox">
                  <div class="popup">
                    <div
                      class="option-item"
                      @click.stop="handleOpenFolder(record.id)"
                      :num="index"
                    >
                      查看
                    </div>
                    ...
                    <div
                      class="option-item"
                      @click.stop="handleDelete(record.id, 'folder')"
                      :num="index"
                    >
                      删除
                    </div>
...
const data = reactive({
  ...
  relatedObjectAttributeValue: {
    value: "10010000-0000-0000-0000-000000000011",
    name: "相册文件",
  },
...
const getQuery = () => {
  ...
  proxy.$post(url, obj).then((res) => {
    if (
      ...
      res.actions[0].returnValue &&
      res.actions[0].returnValue.nodes
    ) {
