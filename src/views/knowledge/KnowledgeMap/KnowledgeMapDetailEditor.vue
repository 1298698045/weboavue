<template>
  <div class="KnowledgeMapDetailEditorWrap">
                        ...
                        <div class="panel-bd panel-bd2">
                          ...
                          >
                              ...
                              <template v-if="column.key === 'Action'">
                                <div class="iconBox">
                                  <div class="popup">
                                    ...
                                    <div
                                      class="option-item"
                                      @click="handleEditSubject(record.id)"
                                      :num="index"
                                    >
                                      编辑
                                    </div>
                            ...
                            <template #expandedRowRender="{ record }">
                              <a-table
                                :columns="columns2"
                                :dataSource="listDataDetail[record.id]"
                                :pagination="false"
                                :rowKey="(record) => record.id"
                              >
                                <template #bodyCell="{ column, index, record }">
                                    ...
                                    <div class="iconBox">
                                      <div class="popup">
                                        <div
                                          class="option-item"
                                          @click="
                                            handlePreview(record.ContentId)
                                          "
                                          :num="index"
                                        >
                                          预览文章
                                        ...
                                        <div
                                          class="option-item"
                                          @click="
                                            handleEditContent(record.ContentId)
                                          "
                                          :num="index"
                                        >
                                          编辑文章
                                        ...
                                        <div
                                          class="option-item"
                                          @click="handlePreview2(record.id)"
                                          :num="index"
                                        >
                                          预览知识地图
                                        </div>
    ...
    <common-form-modal
      ...
      @success="deleteOk"
      :id="data.recordId2"
      :objectTypeCode="objectTypeCode2"
      :entityApiName="sObjectName2"
    ...
    <Lookup-filter
      ...
      :objectTypeCode="objectTypeCode2"
      :field="sObjectName2"
      :lookEntityApiName="sObjectName2"
      @cancel="isLookup = false"
      :filterQuery="lookupFilterQuery"
      @select="selectKbArticle"
    ></Lookup-filter>
  </div>
</template>
<script setup>
...
const getKnowledgeMapData = () => {
  ...
  proxy
    .$post(Interface.list2, {
      ...
      order: "asc",
      displayColumns:
        "Name,ParentSubject,DisplayOrder,Description,CreatedBy,CreatedOn,ModifiedBy,ModifiedOn",
    })
    .then((res) => {
...
const getDetail = () => {
  ...
  proxy.$post(Interface.detail, obj).then((res) => {
    if (
      ...
      formState.Description = fields.Description
...
const selectKbArticle = (e) => {
  ...
  if (e.LIST_RECORD_ID) {
    let index = 1;
    if (
      data.listDataDetail[data.recordId] &&
      data.listDataDetail[data.recordId].length
    ) {
