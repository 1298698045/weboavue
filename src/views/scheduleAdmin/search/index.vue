<template>
  <div class="todoList documentAdmin">
    ...
    <AddSchedule :isShow="isAddSchedule" :id="id" v-if="isAddSchedule" :paramsTime="paramsTime"
      @cancel="isAddSchedule = false" :objectTypeCode="objectTypeCode" :entityApiName="sObjectName"
      @selectVal="refreshScheduleVal" :calendarType="''" />
    <ScheduleDetailModal ref="ScheduleDetailModal1" :isShow="isScheduleDetail" v-if="isScheduleDetail" :id="id"
      @cancel="isScheduleDetail = false" @selectVal="refreshScheduleVal" @handleDelete="handleDelete"
      @edit="handleOpenEdit" />
    <Delete :isShow="isDelete" :desc="deleteDesc" @cancel="cancelDelete" @ok="refreshScheduleVal"
...
const { layoutName, entityType, hightSearchParams, paramsTime, objectTypeCode, sObjectName, isDelete, deleteDesc, external, CalendarActionsConfig, isScheduleDetail, isCollapsed, tableHeight, fieldNames, tabs, activeKey, isModal, isCirculation, searchVal,
        ...
        if (item.name == 'Subject') {
          columnslist.push({
            ...
            formatter: function formatter(value, row, index) {
              ...
              return `<a class="namefield" title="` + val + `" href="javascript:void(0)" onclick="handleDetail('${row.id}')">${val}</a>`;
            }
          });
        }
        else {
...
watch(() => route, (newVal, oldVal) => {
  if (gridRef && gridRef.value && gridRef.value.loadGrid != 'undefined' && !route.params.sObjectName) {
    if (route.path == '/lightning/o/calendar/search') {
      //getTreeData();
      data.queryParams = {
        filterId: '',
        objectTypeCode: '4200',
      ...
      data.entityType = '00V';
      data.layoutName = 'ActivityPointerSearch'
      setTimeout(function () {
        getTabs();
      }, 1000)
    }
