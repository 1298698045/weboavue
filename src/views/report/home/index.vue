<template>
    <div class="wrapper">
        <div class="reportView">
                ...
                <div class="centerRegion">
                    <div class="bodyContainer">
                        <div class="sidebarContainer">
                            <div class="folderSidebar">
                                <nav class="nav-vertical">
                                    <div class="nav-vertical__section navItems entityItems" v-for="(item, index) in navSections" :key="index">
                                        <h2 class="text-title_caps">{{item.label}}</h2>
                                        <ul>
                                            <li class="nav-vertical__item" :class="{'active':row.id==queryScope}" v-for="(row, idx) in item.items" :key="idx">
                                                <a href="javascript:void(0);" :title="row.label" class="nav-vertical__action" @click="handleTabNav(row)">{{row.label}}</a>
                                            </li>
                                        </ul>
                                    </div>
                                </nav>
                        ...
                        <div class="listViewContainer" ref="listViewRef">
                            <Dtable ref="gridRef" name="datagridFilter" :queryParams="queryParams" :columns="columns" :gridUrl="gridUrl" :tableHeight="tableHeight" :loadFilter="loadFilter"></Dtable>                       
                        </div>
                    </div>
                </div>
            </div>
            <div class="defaultMenu" ref="defaultMenuRef" :style="menuStyle" v-show="isDefauleMneuActions" @click.stop>
                <div class="menuActions" style="padding: 10px 0;border: 1px solid #333;border-radius: 4px;">
                    ...
                    <div class="menuItem" @click="handleDelete">删除</div>
        ...
        <CreateReport  v-if="isCreateReport" :isShow="isCreateReport" @cancel="isCreateReport=false" @ok="createReport" />
        <Delete v-if="isDelete" :isShow="isDelete" sObjectName="Report" :recordId="rowId" @cancel="isDelete=false" @ok="loadGrid" desc="是否确定要删除此报表？" />
    </div>
</template>
<script setup>
    import {
