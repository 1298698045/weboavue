<template>
      ...
      <div class="center">
        <div class="bd">
          ...
          <div class="containerWrap">
            <div class="leftWrap">
                <div class="treeBox">
                    <div class="treeBox-head">执行部门</div>
                    <div class="treeBox-bd">
                        <a-tree block-node :tree-data="deptTree">
                            <template #title="{ text }">
                                {{text}}
                            </template>
                        </a-tree>
                ...
                <div class="treeBox">
                    <div class="treeBox-head">合同分类</div>
                    <div class="treeBox-bd">
                        <a-tree block-node :tree-data="contractTree">
                            <template #title="{ text }">
                                {{text}}
                            </template>
                        </a-tree>
  ...
  const getTree = (entity, field) => {
    proxy.$get(Interface.contract.tree,{
        entity: entity
    }).then(res=>{
        data[field] = formTreeData(res.rows,'id','pid');
    })
  }
  getTree('organizationtree', 'deptTree');
  getTree('contractcategory', 'contractTree');
  onMounted(() => {});
  ...
  const loadGrid = (columns) => {
    ...
    $("#datagrid").datagrid({
      url: Interface.contract.admin.list,
      method: "get",
      columns: [columns],
      queryParams: {
  ...
  const getConfig = () => {
    ...
    proxy.$get(Interface.contract.admin.config, {}).then((res) => {
      console.log(res, "res");
  ...
  const getActions = () => {
    proxy
      .$get(Interface.contract.admin.handleActions, {
        objectTypeCode: 1010,
      })
      .then((res) => {
        var listViewActions =
