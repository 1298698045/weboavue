  ...
  <div class="writeEmail">
            ...
            <div class="formWrap">
              <a-form ref="formRef" :model="formState" :label-col="labelCol">
                <a-form-item
                  ...
                  :rules="[
                    {
                      required: activeKey * 1 == 1 ? true : false,
                      message: '请选择收件人',
                    },
                  ]"
                >
                  <a-select
                    ...
                    "
                    ref="select"
                    v-model:value="formState.addressee"
                    mode="multiple"
                    placeholder="请选择收件人"
                  >
                    <a-select-option
                      v-for="(item, index) in selectConcatsList"
                      :key="index"
                      :value="item.ID"
                      class="userclass"
                    >
                ...
                <a-form-item
                  ...
                  :rules="[
                    {
                      required: activeKey * 1 == 2 ? true : false,
                      message: '请选择群组',
                    },
                  ]"
                >
              ...
              <div class="filesWrap">
                <div class="selectFile">
                  <a-upload
                    ...
                    :headers="headers"
                    :data="uploadData"
                    :action="Interface.email.upload"
                    :showUploadList="false"
                    :before-upload="beforeUpload"
                    @change="handleChange"
                  >
                    <a-button type="link"
                      >添加附件<span style="color: red"
                  ...
                  <div class="inboxFileList">
                    ...
                    >
                      <div class="leftImg">
                        ...
                        />
                        <img
                          src="/src/assets/img/filetype/TXT.png"
                          v-else-if="
        ...
        <div class="rightContainer" v-if="activeKey == '1'">
          ...
          <div class="treePeopleWrap">
            <a-tree
              blockNode
              @select="selectPeople"
              v-model:expandedKeys="latelyexpandedKeys"
              v-model:selectedKeys="latelySelectedKeys"
              :tree-data="latelyTreeData"
            >
            </a-tree>
            <a-tree
              blockNode
    ...
    <div class="writeFooter">
      <div class="footerOption">
        <a-button class="mr10" type="primary" @click="handleSendEmail(1)"
          >发送</a-button
        >
...
const handleChange = (file) => {
  if (file.file.status == "done") {
    ...
    if (
      ...
      file.file &&
      file.file.response &&
      file.file.response.actions &&
      file.file.response.actions[0] &&
      file.file.response.actions[0].returnValue &&
      file.file.response.actions[0].returnValue[0]
    ) {
      ...
      data.fileList2.push({
        ...
        url: file.file.url,
        fileExtension:
          file.file.response.actions[0].returnValue[0].fileExtension,
...
const highlightKeyword = (label) => {
  ...
  var highlightedLabel = label.replace(
    ...
    if (res && res.nodes) {
      for (var i = 0; i < res.nodes.length; i++) {
        ...
        for (var cell in item) {
          ...
          if (cell == "AvatarImg") {
            item[cell] =
              girdFormatterValue(cell, item) ||
              require("@/assets/img/avatar-r.png");
          }
        }
        if (!item.AvatarImg) {
    ...
    if (treeNode.id && treeNode.id != "") {
      loadGroupPeople(treeNode.id).then((res) => {
        ...
        let temp = res.nodes.map((item) => {
          item.title =
            item.RegardingObjectIdName && item.RegardingObjectIdName.textValue
              ? item.RegardingObjectIdName.textValue
              : "";
          item.key = item.id;
          item.isLeaf = true;
          return item;
...
const loadDeptUser = (deptId) => {
  return new Promise((resolve) => {
    ...
    let d = {
      ...
      displayColumns:
        "id,Name,FullName,UserName,EmployeeId,BusinessUnitIdName,OrganizationId",
...
const getUserData = (params) => {
  ...
  data.isRadioUser = false;
  if (params.id) {
    let index = data.selectConcatsList.findIndex(
...
const handlePreviewFile = (item) => {
  ...
  if (
    ...
    item.viewUrl = "/lightning/r/office/view?id=" + item.id + "&docType=20022";
    openControlViewFile(
      item.id,
      item.createdByName,
      item.fileExtension,
...
const deleteFile = (item) => {
  ...
  proxy
    .$post(Interface.email.deleteAttachment, obj)
    .then((res) => {
      if (
        res &&
        res.actions &&
